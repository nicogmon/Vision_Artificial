...
const std::vector<std::vector<std::string>> gym = {
  {"sports ball", "cup", "chair", "sports ball"},
  {"sofa"}
};
...
std::set<string> near_objects;//habria que guardar distancia de alguna forma
...
std::vector<cv::Point3d> point_3d(std::shared_ptr<image_geometry::PinholeCameraModel> camera_model_, const cv::Mat& out_image_depth, std::vector <cv::Point2f> points2d, geometry_msgs::msg::TransformStamped transform2 ){
  ...
  for (int i = 0; i < (int)points2d.size(); i++){
    ...
    cv::Point3d pt(point3d_transformed.at<double>(0), point3d_transformed.at<double>(1), point3d_transformed.at<double>(2));
    pts.push_back(pt);
  }
...
void localitation(){
  ...
  for (room it : rooms){
    ...
    std::cout << "Habitacion counter: " << to_string(room_counter) << "\n";
...
const
{
  ...
  if (option == 0){
    ...
    std::vector<cv::Point3d> pts0_3d = point_3d(camera_model_, out_image_depth, p0, transform2);
    std::vector<cv::Point3d> pts1_3d = point_3d(camera_model_, out_image_depth, p1, transform2);
    ...
    for (uint i = 0; i < p0.size(); i++) {
      ...
      if (status[i] == 1) {
        ...
        // std::cout << "Punto 0: " << pts0_3d[i] << " Punto 1: " << pts1_3d[i] << "\n";
        // std::cout << "Diferencia: " << pts1_3d[i] - pts0_3d[i] << "\n";
        // std::cout << "Distancia a p0[i]: " << sqrt(pow(pts0_3d[i].z,2) + pow(pts0_3d[i].y,2)) << "\n";
        // std::cout << "Distancia a p1[i]: " << sqrt(pow(pts1_3d[i].z,2) + pow(pts1_3d[i].y,2)) << "\n";
        // std::cout << "Pendiente: " << (p1[i].y - p0[i].y) / (p1[i].x - p0[i].x) << "\n";
    ...
    if (classes.empty()){
      ...
      string classesFile = "/home/nicogmon/Vision/vis_ws/src/practicafinal-grupo15/src/practicafinal-grupo15/cfg/coco.names";
      ...
      string modelConfiguration = "/home/nicogmon/Vision/vis_ws/src/practicafinal-grupo15/src/practicafinal-grupo15/cfg/yolov3.cfg";
      string modelWeights = "/home/nicogmon/Vision/vis_ws/src/practicafinal-grupo15/src/practicafinal-grupo15/cfg/yolov3.weights";
